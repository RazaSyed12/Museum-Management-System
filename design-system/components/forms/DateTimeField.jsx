...
export function DateTimeField({ type = 'date', invalid, disabled, id, style, ...rest }) {
  const [focus, setFocus] = React.useState(false);
  return (
    <span style={{ position: 'relative', display: 'block' }}>
      <input id={id} type={type} disabled={disabled} aria-invalid={invalid || undefined}
        onFocus={() => setFocus(true)} onBlur={() => setFocus(false)}
        style={{ ...controlStyle({ invalid, focus, disabled }), paddingRight: 40, ...style }} {...rest} />
      <Icon name={type === 'time' ? 'clock' : 'calendar-days'} size={18} color="var(--text-muted)"
